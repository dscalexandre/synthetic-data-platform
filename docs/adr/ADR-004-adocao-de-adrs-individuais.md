...
Manter todas as decisões em um único arquivo faz com que alterações em decisões
distintas compartilhem o mesmo histórico de versionamento, dificultando
identificar quando cada decisão foi proposta, aprovada, revisada, substituída ou
...
Os arquivos devem ser referenciados pela documentação técnica quando sustentarem
decisões descritas em `docs/architecture.md` ou no `README.md`.
